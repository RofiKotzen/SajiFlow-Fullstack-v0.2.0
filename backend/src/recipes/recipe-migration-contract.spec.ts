...
describe("Recipe Phase 1 migration contracts", () => {
  const migration = readFileSync(resolve(process.cwd(), "drizzle/0005_recipe_food_cost_phase1.sql"), "utf8");
  const service = readFileSync(resolve(process.cwd(), "src/recipes/recipes.service.ts"), "utf8");
  ...
  it("selects UUID audit actors deterministically without unsupported UUID aggregates", () => {
    expect(migration).not.toMatch(/(?:min|max)\s*\(\s*r\.(?:created_by|updated_by)\s*\)/i);
    expect(migration).toContain("array_agg(r.created_by ORDER BY r.created_at ASC, r.id ASC)");
    expect(migration).toContain("array_agg(r.updated_by ORDER BY r.updated_at DESC, r.id DESC)");
  });
  ...
  it("recalculates approval costing inside the approval transaction", () => {
    const approval = service.slice(service.indexOf("async approve"), service.indexOf("async revise"));
    expect(approval).toContain('this.calculate(tx, actor, draft.id, outletId, "approval_snapshot")');
  ...
  it("does not modify Inventory, PO, GR, order, or KDS rows", () => {
    for (const table of ["stock_batches", "stock_movements", "purchase_orders", "goods_receipts", "order_items", "kitchen_ticket_items"])
      expect(migration).not.toMatch(new RegExp(`(?:update|delete\\s+from)\\s+public\\.${table}`, "i"));
  });
});
