...
export class UsersService {
  ...
  async create(actor: AuthUser, dto: CreateUserDto) {
    ...
    await this.audit.record({
      ...
      entityType: "user",
      entityId: created.id,
      afterData: { ...created, password: undefined },
    });
  ...
  async resetPassword(actor: AuthUser, id: string, dto: ResetPasswordDto) {
    ...
    await this.database.db.transaction(async (tx) => {
      const [credential] = await tx
        ...
        .where(
          and(
            eq(userCredentials.userId, id),
            eq(userCredentials.tenantId, actor.tenantId),
          ),
        )
        .returning({ userId: userCredentials.userId });

      if (!credential)
        throw new NotFoundException("Kredensial user tidak ditemukan");
      ...
      await tx
        ...
        .where(
          and(
            ...
            eq(refreshTokens.tenantId, actor.tenantId),
  ...
  async assignRoles(actor: AuthUser, userId: string, dto: AssignRolesDto) {
    ...
    if (roleIds.length) {
      ...
      if (validRoles.length !== roleIds.length)
        throw new NotFoundException("Ada role yang bukan milik tenant");
    }
    if (outletIds.length) {
      const validOutlets = await this.database.db
      ...
      if (validOutlets.length !== outletIds.length)
        throw new NotFoundException("Ada outlet yang bukan milik tenant");
    }
    const now = new Date();
    await this.database.db.transaction(async (tx) => {
      await tx
        .update(userRoles)
        .set({ validUntil: now, updatedBy: actor.userId })
        .where(
          and(
