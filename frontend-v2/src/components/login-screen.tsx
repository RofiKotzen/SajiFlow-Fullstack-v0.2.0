  ...
  return (
    <main className="grid min-h-screen bg-cream lg:grid-cols-[1.15fr_1fr]">
      <section className="relative hidden overflow-hidden bg-pine p-12 text-cream lg:flex lg:flex-col lg:justify-between">
        <div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-xl bg-pine-soft ring-1 ring-white/15"><span className="text-lg font-semibold">S</span></div><div><strong className="block text-lg">Saji Flow</strong><span className="mono text-xs text-pine-mute">sistem operasional restoran</span></div></div>
        ...
        <div className="mono flex items-center gap-3 text-xs text-pine-mute"><span>Rencanakan</span><i className="h-px flex-1 bg-pine-line"/><span>Beli</span><i className="h-px flex-1 bg-pine-line"/><span>Simpan</span><i className="h-px flex-1 bg-pine-line"/><span>Jual</span><i className="h-px flex-1 bg-pine-line"/><span>Sajikan</span></div>
      </section>
      <section className="flex items-center justify-center px-5 py-10 sm:px-10">
        <form className="w-full max-w-md rounded-2xl bg-card p-7 shadow-[0_18px_60px_oklch(0.28_0.02_125/0.08)] ring-1 ring-black/5 sm:p-9" onSubmit={submit}>
          ...
          <div className="space-y-4">
            ...
            <label className="block text-sm font-medium">Kata sandi<input className="mt-1.5 h-11 w-full rounded-lg bg-white px-3 text-sm ring-1 ring-black/10 outline-none transition focus:ring-2 focus:ring-olive/40" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" minLength={8} maxLength={128} required disabled={submitting}/></label>
          </div>
          {error && <div className="mt-4 rounded-lg bg-terra/10 px-3 py-2.5 text-sm leading-relaxed text-terra" role="alert">{error}</div>}
          <button className="mt-6 inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-olive px-4 text-sm font-semibold text-cream transition hover:bg-olive-deep disabled:cursor-wait disabled:opacity-60" type="submit" disabled={submitting}>{submitting && <LoaderCircle className="size-4 animate-spin"/>}{submitting ? "Menghubungkan…" : "Masuk ke ruang kerja"}</button>
        </form>
      </section>
    </main>
  );
...
function loginErrorMessage(error: unknown): string {
  if (!(error instanceof ApiError)) return "Gagal masuk. Silakan coba lagi.";
  if (error.kind === "locked") return error.message;
  if (error.kind === "validation") return `Data untuk masuk belum valid: ${error.message}`;
  if (error.kind === "network" || error.kind === "unavailable") return error.message;
  if (error.kind === "unauthorized") return "Kode tenant, email, atau kata sandi tidak sesuai, atau akun tidak aktif.";
